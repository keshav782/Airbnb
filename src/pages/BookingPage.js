...
export const BookingPage = () => {
  ...
  useEffect(() => {
    if (id) {
      axios.get("/booking").then((res) => {
        const booking = res.data.find(({ _id }) => _id === id);
        if (booking) {
          setBooking(booking);
        }
      });
        ...
        <div className="bg-black p-8 grid gap-4">
          ...
          {booking.place?.photos?.length > 0 &&
            booking.place.photos.map((photo) => (
              <div>
                <img className="w-full" src={'http://localhost:5000/uploads/'+photo} alt="" />
              </div>
            ))}
        </div>
      </div>
  ...
  return (
    <div className="my-8">
      ...
      <a
        className="my-2 block flex gap-2"
        target="_blank"
        rel="noreferrer"
          ...
          <div className="grid">
            {booking.place.photos?.[1] && (
              <img
                onClick={() => setShowAllPhotos(true)}
                className="aspect-square cursor-pointer object-cover"
                src={'http://localhost:5000/uploads/'+booking.place.photos[1]}
                alt=""
              />
            )}
            <div className="overflow-hidden">
              {booking.place.photos?.[2] && (
                <img
                  ...
                  src={'http://localhost:5000/uploads/'+booking.place.photos[2]}
                  alt=""
                />
              )}
            </div>
